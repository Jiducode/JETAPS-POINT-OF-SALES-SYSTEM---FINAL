...
$selectLowStock = $pdo->prepare("SELECT * FROM tbl_product WHERE pstock < 100 AND pstock > 0 ORDER BY pid DESC");
$selectLowStock->execute();

$selectNoStock = $pdo->prepare("SELECT * FROM tbl_product WHERE pstock = 0 ORDER BY pid DESC");
$selectNoStock->execute();
...
while ($row = $selectNoStock->fetch(PDO::FETCH_OBJ)) {
    $quantity = $row->pstock;
...
<li class='nav-item dropdown'>
    <a href='productlist.php' class='nav-link dropdown-toggle' id='lowStockDropdown' role='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>
        <i class='fas fa-exclamation-triangle '> Low on stock</i>
...
while ($row = $selectLowStock->fetch(PDO::FETCH_OBJ)) {
    ...
    if ($quantity < 100 && $quantity >= 0) {
        echo "<a class='dropdown-item' href='productlist_user.php'>$row->pname</a>";
    }
}
...
// Second bell icon for products with zero stock
echo "
<li class='nav-item dropdown'>
    <a href='productlist.php' class='nav-link dropdown-toggle' id='noStockDropdown' role='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>
        <i class='fas fa-bell'> No stock</i>
        " . ($noStockCount >= 0 ? "<span class='badge badge-danger'>$noStockCount</span>" : "") . "
    </a>
    <div class='dropdown-menu' aria-labelledby='noStockDropdown'>
        <h6 class='dropdown-header'>No Stock</h6>";

// Display the products with zero stock in the dropdown
$selectNoStock->execute();
while ($row = $selectNoStock->fetch(PDO::FETCH_OBJ)) {
    $quantity = $row->pstock;
