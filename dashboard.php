...
$query = "SELECT i.order_date, i.total, i.subtotal, i.discount, SUM(id.qty) as total_products
          FROM tbl_invoice i
          LEFT JOIN (
          ...
          GROUP BY i.order_date, i.paid, i.subtotal, i.discount";
$result = mysqli_query($connect, $query);
$chart_data = '';
while ($row = mysqli_fetch_array($result)) {
    $chart_data .= "{ order_date:'" . $row["order_date"] . "', total:" . $row["total"] . ", subtotal:" . $row["subtotal"] . ", discount:" . $row["discount"] . ", total_products:" . $row["total_products"] . "}, ";
}
...
var chart = Morris.Bar({
    ...
    data: chartData,
    xkey: 'order_date',
    ykeys: ['total', 'subtotal', 'discount', 'total_products'],
    labels: ['Total Sales', 'Subtotal', 'Discount', 'Total Products Sold'],
    hideHover: 'auto',
    stacked: true
});
